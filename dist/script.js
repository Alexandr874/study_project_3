    ...
    value: function initMask() {
      ...
      var setCursorPosition = function setCursorPosition(pos, elem) {
        ...
        if (elem.setSelectionRange) {
          //Данный метод (setCursorPosition) используется для 
          elem.setSelectionRange(pos, pos); //установки позиции курсора внутри элемента формы (input) при вызове
        } else if (elem.createTextRange) {
          //Метод (createTextRange) является методом объекта HTMLInputElement и   
          var range = elem.createTextRange(); //используется для создания объекта TextRange, который представляет 
          //собой диапазон текста внутри элемента формы (input).
          //Объект (TextRange) используется для работы с выделением и манипуляциями
          //текстом в старых версиях Internet Explorer, которые не поддерживают метод 
          //setSelectionRange или поддерживают его частично. Этот метод является 
          // устаревшим и обычно не используется в современных браузерах.
...
function () {
  function VideoPlayer(triggers, overlay) {
    ...
    this.onPlayerStateChange = this.onPlayerStateChange.bind(this); // при помощи метода (bind)
    // привязываем контекст вызова 
  }
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; // получили соседа 
          // по верстке, тоесть кнопку видеоплеера, который находится ниже

          if (i % 2 == 0) {
            //что бы срабатывал каждый второй элемент(второй видео плееер)
            blockedElem.setAttribute('data-disabled', 'true'); // и повесили на него атрибут
          }
        } catch (e) {}
    ...
    value: function onPlayerStateChange(state) {
      try {
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // получили соседа 
        // по верстке, тоесть кнопку видеоплеера, который находится ниже

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // при помощи метода 
        // (cloneNode(true)) скопировали ноду кнопки(копирование должно быть глубокое, поэтому ставим (true))
        // и теперь эта кнопка будет менять, кнопку на которой находился замок
...
function (_Slider) {
  ...
  function MainSlider() {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(MainSlider).apply(this, arguments));
  }
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return MiniSlaider; });
/* harmony import */ var core_js_modules_es_symbol__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! core-js/modules/es.symbol */ "./node_modules/core-js/modules/es.symbol.js");
/* harmony import */ var core_js_modules_es_symbol__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_es_symbol__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var core_js_modules_es_symbol_description__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! core-js/modules/es.symbol.description */ "./node_modules/core-js/modules/es.symbol.description.js");
/* harmony import */ var core_js_modules_es_symbol_description__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_es_symbol_description__WEBPACK_IMPORTED_MODULE_1__);
...
/*#__PURE__*/
function (_Slider) {
  _inherits(MiniSlaider, _Slider);

  function MiniSlaider(container, next, prev, activeClass, animate, autoplay) {
    _classCallCheck(this, MiniSlaider);

    return _possibleConstructorReturn(this, _getPrototypeOf(MiniSlaider).call(this, container, next, prev, activeClass, animate, autoplay));
  }

  _createClass(MiniSlaider, [{
    key: "decorizeSlides",
    ...
    value: function moveButtonsToEnd() {
      ...
      this.slides.forEach(function (slide, i) {
        if (slide.tagName === "BUTTON") {
          _this2.container.appendChild(_this2.slides[i]);
        }
      });
    }
  }, {
    ...
    value: function bindTriggers() {
      ...
      this.prev.addEventListener('click', function () {
        ...
        _this3.container.insertBefore(active, _this3.slides[_this3.slides.length - 1]);
